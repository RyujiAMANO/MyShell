	...
	public function createTest($param, $action = null) {
		...
		if ($this->isManagerPlugin()) {
			$output .=
				$this->_classMethod(
					...
					'__assertEditGet()',
					array(
						'$this->assertInput(\'form\', null, \'/' . Inflector::underscore($this->plugin) . '/' . $controller . '/edit\', $this->view);',
		...
		if ($this->isManagerPlugin()) {
			$process = array(
				...
				chr(9) . chr(9) . 'array(\'action\' => \'edit\'), null, \'view\');',
				'',
				'//チェック',
		...
		if ($this->isManagerPlugin()) {
			$process = array(
				...
				chr(9) . chr(9) . 'array(\'action\' => \'edit\'), null, \'view\');',
				'//$this->_testPostAction(\'put\', $this->__data(),',
				'//' . chr(9) . chr(9) . 'array(\'action\' => \'edit\'), \'BadRequestException\', \'view\');',
				'',
				'//TODO:必要に応じてassert書く',
			);
		} else {
